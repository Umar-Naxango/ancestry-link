      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#1a1a1a] rounded-3xl shadow-sm overflow-hidden border dark:border-gray-800">
          <div className="h-48 bg-gradient-to-r from-emerald-600 to-teal-600 relative">
            <div className="absolute -bottom-16 left-8">
              <img src={currentUser.picture} alt={currentUser.name} className="w-32 h-32 rounded-3xl border-4 border-white object-cover shadow-lg" />
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-3xl p-6 shadow-sm border dark:border-gray-800">
          <div className="flex justify-between mb-6">
            ...
            {isMale && spouses.length < 4 && <button onClick={() => setIsSpouseModalOpen(true)} className="text-emerald-600 dark:text-emerald-400 text-sm font-medium">+ Add Spouse</button>}
          </div>
          {spouses.length === 0 ? (
            ...
            spouses.map(sp => (
              <div key={sp.id} className="border dark:border-gray-800 rounded-2xl p-5 mb-3 last:mb-0">
                <div className="flex items-center gap-3">
                  <img src={sp.picture} alt={sp.name} className="w-12 h-12 rounded-full object-cover" />
                  <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {children.map(child => (
              <div key={child.id} className="border dark:border-gray-800 rounded-3xl p-6 hover:shadow-md transition">
                <img src={child.picture} alt={child.name} className="w-20 h-20 rounded-2xl mx-auto mb-4 object-cover" />
